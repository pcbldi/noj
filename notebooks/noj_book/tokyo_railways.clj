...
;; So, `:elements` can either be a list of nodes and edges:

(def mixed-elements
  ; a node has :group "nodes"
  [{:group "nodes"
    :data {:id "a"}
    :position {:x 0 :y 0}}
   {:group "nodes"
...
;; Or it can be a map, keyed by group:
...
(kind/cytoscape {:elements separate-elements
                 :style simple-style
                 :layout simple-layout})
